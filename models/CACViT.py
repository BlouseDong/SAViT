...
#     def __init__(self, channel, h, w, reduction):
...
class SupervisedMAE(nn.Module):
    ...
    def __init__(self, img_size=384, patch_size=16, in_chans=3, 
        ...
        # self.CA1 = CA_Block(channel=3, h=384, w=384,reduction=3)
        # self.CA2 = CA_Block(channel=4, h=64, w=64, reduction=4)

        self.My1 = MDFA(dim_in=3, dim_out=3)
        self.My2 = MDFA(dim_in=4, dim_out=4)
    ...
    def AttentionEnhance(self, attns, l=24, n=1):
        ...
        # 转换为图像并保存
        # attn_image = attns_x2y.squeeze(0).squeeze(0).detach().cpu().numpy()  # 假设这是一个单通道图像
        # plt.figure(figsize=(10, 10))
        # plt.imshow(attn_image, cmap='viridis')
        # plt.colorbar()
        # plt.title("Enhanced Attention Map")
        #
        # filepath = "D:\\zhiwei\\CACViT\\image\\attention_enhanced.png"
        # # 确保目标文件夹存在
